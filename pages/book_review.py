...
books = df_top100_books["book title"].unique() #unique faz uma lista sem repeticao dos nomes
book = st.sidebar.selectbox('Books',books)

df_book = df_top100_books[df_top100_books["book title"]== book]
df_reviews_f = df_reviews[df_reviews["book name"]== book]

book_title = df_book["book title"].iloc[0]#pra pegar so o nomes
book_gender = df_book["genre"].iloc[0]
book_price = f"${df_book['book price'].iloc[0]}"
book_rating = df_book["rating"].iloc[0]
book_year = df_book["year of publication"].iloc[0]

st.title(book_title)
st.subheader(book_gender)
